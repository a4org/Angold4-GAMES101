...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        //Canonical cube to screen space
        for (auto & vert : v)
        {
		...
		for (int j = y_min; j <= y_max; j++)
		{
			if (insideTriangle(i + 0.5, j + 0.5, t.v))
			{
				...
				if (zp < depth_buf[get_index(i, j)])
				{
                    ...
                    // pass these attributes to fragment shader to get the final color
                    // fragment_shader = the shader in main() which we passed into (default phong shader)
